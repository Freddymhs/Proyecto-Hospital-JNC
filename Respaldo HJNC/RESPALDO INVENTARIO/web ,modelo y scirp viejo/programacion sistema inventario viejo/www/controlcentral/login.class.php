...
class Login
{
	...
	{	
		...
		$this->user = 'operador' || 'c0ntr0l';
		$this->pass = 'usuario' || 'usu1rio';
		$this->user_id = 1;
		...
		if($_SERVER['REQUEST_METHOD']=='POST')
		{
			...
			if($u&&$p)
			{
				...
				if($permit)
				{
					...
					if($u=='operador' && $p=='c0ntr0l'){
					
						// json response 
						 $m = '{'.
						 'succes: true,'.
						 'title: \'<strong>Logeo correcto</strong>\', '.
						 'content: \'<script>window.location = "menu.php";</script>\''.
						 '}';
					}
					if($u=='usuario' && $p=='usu1rio'){
						
						// json response 
						 $m = '{'.
						 'succes: true,'.
						 'title: \'<strong>Logeo correcto</strong>\', '.
						 'content: \'<script>window.location = "reportes.php";</script>\''.
						 '}';
					}
					echo $m;
