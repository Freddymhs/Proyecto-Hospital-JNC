										...
										<table border="1" class="table table-bordered table-striped text-center">
											<thead>
												<tr>
													...
													<td class="border border-success">Modelo Monitor</td>
													<td class="border border-success">Disponibilidad</td>
													<td class="border border-success">Fin del arriendo</td>
												</tr>
											</thead>
											<tbody>
												<?php foreach ($monitor as $value): ?>
													<tr>
														<td><b><?= $value->SERIALMONITOR ?></b></td>
														<td><b><?= $value->MODELOMON ?></b></td>	
														<?php if ($value->DISPONIBILIDAD == '1'): ?>
															...
															<!-- x -->
															<?php if ($value->FINARRIENDOMONITOR != "0000-00-00" && $value->FINARRIENDOMONITOR  != ""): ?>
																<td> <b><?= $value->FINARRIENDOMONITOR  ?> </b></td>
																<?php else: ?>
																	<td><b>N/A</b></td>	
																<?php endif ?>
																<!-- x-->	
