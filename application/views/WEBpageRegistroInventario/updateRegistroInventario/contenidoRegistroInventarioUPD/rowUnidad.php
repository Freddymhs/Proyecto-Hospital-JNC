			...
			<?php if ((count($listunidades)) > 0): ?>
				<select  class="form-control"  name="selectunidades" id="selectunidades" onclick="cleanUnidad();" >
					...
					<?php foreach ($listunidades as $value): ?>
						...
						<?php if ( $fila[0]->NOMBREUNIDAD  === $value->NOMBREUNIDAD): ?>
							<option selected value="<?= $value->NOMBREUNIDAD ?>" <?php echo set_select('selectunidades', $value->NOMBREUNIDAD); ?> ><?= $value->NOMBREUNIDAD ?></option>							
							<?php else: ?>
