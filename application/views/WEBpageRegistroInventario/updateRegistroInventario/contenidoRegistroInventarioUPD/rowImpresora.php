...
<div class="row"> 
	<div class="col-3">
		<label class="col-form-label"  for="xUsuario">Modelo de Impresora</label>
	</div>
	<div class="col-4">
		...
		<div class="input-group mb-3 input-group">
			...
			<?php if ((count($listimpresoras)) > 0): ?>
				<select class="form-control" name="selectImpresora" id="selectImpresora" onmouseleave="cleanNewImp()">
					...
					<?php foreach ($listimpresoras as $value): ?>
						...
						<?php if ($value->MODELOIMP == $DATOSIMP[0]->MODELOIMP): ?>
							<option selected value="<?= $value->MODELOIMP ?>" <?php echo set_select('selectImpresora', $value->MODELOIMP); ?> ><?= $value->MODELOIMP ?></option>
							<?php else: ?>
			...
			<div class="col-2">
				<label class="col-form-label"  for="uyt">Serial de Impresora</label>
			</div>
			<div class="col-3">
				<div class="input-group mb-3 input-group">
					
					<?php if (isset($DATOSIMP[0]->SERIALIMPRESORA)): ?>
						<input  type="text" name="txtserialimpresora" id="txtserialimpresora"  class="form-control" value="<?= null !=(set_value('txtserialimpresora'))? set_value('txtserialimpresora') : $DATOSIMP[0]->SERIALIMPRESORA?>">
						<?php else: ?>
							<input  type="text" name="txtserialimpresora" id="txtserialimpresora"  class="form-control" value="<?= null !=(set_value('txtserialimpresora'))? set_value('txtserialimpresora') : '' ?>">
						<?php endif ?>
					</div>
					<div class="input-group mb-3 input-group">
						<!-- formerrror -->
			...
			<div class="col-12">
				...
				<div class="collapse border border-white" id="collapseImpresora">
					<div class="card card-header bg-primary text-white">
						<p class="align-self-center" style="font-size: 15px;"><b>Crear / Actualizar Modelo de Impresora</b></p>
					</div>
