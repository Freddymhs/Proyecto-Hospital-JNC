...
<div class="card align-self-center">
	<table id="example" class="align-self-center table  table-bordered table-responsive" cellspacing="0" style="width:100%">
		...
		<thead class="bg bg-primary text-white">
			<tr>
				<th>Anexo</th>
				<th class="bg bg-secondary text-white-50">Detalles de la Persona</th>
		...
		<tbody>
			<?php $persona = $this->db->query("select * from empleados")->result(); ?>
			<?php $inv  = $this->db->query("select * from inventario_soporte_hjnc")->result(); ?>
			<?php $r = $this->db->query("SELECT * FROM `telefono`")->result(); ?>
			...
			<?php foreach ($r as $qt): ?>
				<tr>
					...
					<td>
						<?php	for($i=0; $i<sizeof($inv) ; $i++){
							$ruta = base_url("index.php/controllerEmpleados/informacion/{$inv[$i]->RUT}"); 
							if ($qt->NROANEXO == $inv[$i]->NROANEXO) {


								echo "<a href=".$ruta.">" .$inv[$i]->RUT."</a>"."<br>";
