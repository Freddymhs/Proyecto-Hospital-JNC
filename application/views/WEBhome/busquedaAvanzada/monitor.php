	...
	<table id="example" class="align-self-center table  table-bordered table-responsive" cellspacing="0" style="width:100%">
		...
		<thead class="bg bg-primary text-white">
			<tr>
				...
				<th>Fin Arriendo</th>
				<td class="bg bg-secondary">Modelo</td>
				<td class="bg bg-secondary">funcionalidades</td>
				<td class="bg bg-secondary">resolucion</td>
				<td class="bg bg-secondary">pulgadas</td>
				<td class="bg bg-secondary">marca</td>
				<td class="bg bg-secondary">durabilidad</td>
			...
			<?php foreach ($r as $qt): ?>
				<tr>
					...
					<?php if ($qt->DISPONIBILIDAD == "1"): ?>
						...
						<!-- x -->
						<td><?= $qt->MODELOMON ?></td>
						<td><?= $qt->TIPOFUNCIONALIDADES ?></td>
						<td><?= $qt->RESOLUCION ?></td>
						<td><?= $qt->PULGADAS ?></td>
						<td><?= $qt->MARCA ?></td>
						<td><?= $qt->DURABILIDAD ?></td>
