	...
	<table id="example" class="align-self-center table  table-bordered table-responsive" cellspacing="0" style="width:100%">
		...
		<tbody>
			...
			<?php foreach ($r as $qt) : ?>
				...
				<?php $rutaT = base_url("index.php/controllerTelefono/informacion/{$qt->NROANEXO}");  ?>
				<?php $rutaR = base_url("index.php/controllerEmpleados/informacion/{$qt->RUT}");  ?>
				<tr>
					<td>
					...
					<?php if (strlen($qt->NOMBRES) > 0) : ?>
						...
						<?php if ($qt->dispEMpleados == 1): ?>
							...
							<td><a href="<?= $rutaR ?>"><?= $qt->NOMBRES ?></a></td>
							<!-- xx -->
							...
							<?php else: ?>
								<?php if ($qt->dispEMpleados == 0): ?>
									<td >
										<a href="<?= $rutaR ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->NOMBRES  ?></a>
									</td> 
								<?php endif ?>
									...
									<?php else: ?>
										<?php if ($qt->dispTelefono == 0): ?>
											<td >
												...
												<a href="<?= $rutaT ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->NROANEXO  ?></a>
											</td> 
										<?php endif ?>
									<?php endif ?>
										...
										<?php if ($qt->FINARRIENDOPC != "0000-00-00" && $qt->FINARRIENDOPC  != "") : ?>
											...
											<?php if (strlen($qt->SERIALCOMPUTADOR) > 0) : ?>
												...
												<td> 
													...
													<?php if ($qt->dispPC == 1): ?>
														<a href="<?= $rutaPerfilPC ?>"> <?= $qt->SERIALCOMPUTADOR ?> </a>
														<?php else: ?>
															<?php if ($qt->dispPC == 0): ?>

																<a   href="<?= $rutaPerfilPC ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->SERIALCOMPUTADOR  ?></a>
														...
														<?php if ($qt->FINARRIENDOMONITOR != "0000-00-00" && $qt->FINARRIENDOMONITOR  != "") : ?>
															...
															<?php if ($qt->SERIALMONITOR != null  && $qt->SERIALMONITOR  != "") : ?>
																...
																<?php if ($qt->dispMonitor == 1): ?>
																	<td> <a href="<?= $rutaPerfilMonitor ?>"> <?= $qt->SERIALMONITOR ?> </a> </td>
																	...
																	<?php else: ?>
																		<?php if ($qt->dispMonitor == 0): ?>
																			...
																			<td> 
																				...
																				<a   href="<?= $rutaPerfilMonitor ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->SERIALMONITOR  ?></a>
																	...
																	<?php if ($qt->MODELOIMP != null 	||  strlen($qt->MODELOIMP) > 0) : ?>
																		...
																		<?php if ($qt->FINARRIENDOIMP != "0000-00-00" && $qt->FINARRIENDOIMP  != "") : ?>
																			...
																			<?php if ($qt->SERIALIMPRESORA  != null) : ?>
																				<?php if ($qt->disIMPR == 1): ?>
																					<td> <a href="<?= $rutaPerfilImpresora ?>"> <?= $qt->SERIALIMPRESORA ?> </a> </td>
																					<?php else: ?>
																						<?php if ($qt->disIMPR == 0): ?>
																							<td>
																								<a   href="<?= $rutaPerfilImpresora ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->SERIALIMPRESORA  ?></a>
																							</td>
																					...
																					<?php if ($qt->MODELOUPS  != null) : ?>
																						...
																						<?php if (strlen($qt->SERIALUPS) > 0) : ?>
																							...
																							<?php if ($qt->disUPS == 1): ?>
																								<td> <a href="<?= $rutaPerfilUPS ?>"> <?= $qt->SERIALUPS ?> </a> </td>
																								<?php else: ?>
																									<?php if ($qt->disUPS == 0): ?>
																										<td> 

																											<a   href="<?= $rutaPerfilUPS ?>" style="background-color: #ff6969" class=" bg rounded  text-white around" > <?= $qt->SERIALUPS  ?></a>
																										</td>
																									<?php endif ?>
																								<?php endif ?>
